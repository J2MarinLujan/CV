      ...
      <div class="center-center flex-col">
        <div class="center-center gap-2 sm:gap-5 mb-6">
          <span
            ...
            @click="
              (e) => {
                handleClickCategory(e, index)
              }
            "
            :key="index"
            :class="[
              { 'active-work bg-secondary text-base-content duration-[750ms]': active === index },
...
const projectsFilter = ref<object[]>(projects)

function handleClickCategory(e, index) {
  item.value = { name: e.target.textContent }
  active.value = index
}
